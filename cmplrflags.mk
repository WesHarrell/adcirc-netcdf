...
ifeq ($(compiler),gnu)
  ...
  $(warning (INFO) $(MACHINENAME) found)
  ifneq ($(FOUND),TRUE)
     FOUND := TRUE
  else
     MULTIPLE := TRUE
...
ifeq ($(compiler),gnu)
  ...
  $(warning (INFO) Corresponding compilers and flags found in cmplrflags.mk.)
  $(warning (INFO) Wes!  Using gnu compilers)
  ifneq ($(FOUND),TRUE)
     FOUND := TRUE
  else
     MULTIPLE := TRUE
...
ifeq ($(compiler),intel)
  ...
  ifeq ($(NETCDF),enable)
     ...
     ifeq ($(MACHINENAME),mahuika)
        $(warning (INFO) NeSI!  Using intel compilers. Machine is $(MACHINENAME). NETCDF is $(NETCDF))
        HDF5HOME       :=/opt/nesi/CS400_centos7_bdw/HDF5/1.10.5-iimpi-2020a
        NETCDFHOME     :=/opt/nesi/CS400_centos7_bdw/netCDF/4.7.3-iimpi-2020a
        NETCDFFHOME    :=/opt/nesi/CS400_centos7_bdw/netCDF-Fortran/4.5.2-iimpi-2020a
        FLIBS          := $(FLIBS) -L$(HDF5HOME)/lib -L$(NETCDFFHOME)/lib -L$(NETCDFHOME)/lib -lnetcdff -lnetcdf -lhdf5 -lhdf5_fortran -L../swan
        FFLAGS1    :=$(FFLAGS1) -I$(NETCDFFHOME)/include -I$(NETCDFHOME)/include -I$(HDF5HOME)/include
     endif
  endif
